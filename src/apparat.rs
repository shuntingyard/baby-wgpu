...
use crate::pipelines::{Plumber, SimplePipe};
...
impl State {
    ...
    pub async fn new(window: Window) -> Self {
        ...
        let _brown_shader = device.create_shader_module(wgpu::include_wgsl!("shader.wgsl"));
        let _pretty_shader = device.create_shader_module(wgpu::include_wgsl!("smoothstep.wgsl"));
        let render_pipeline = SimplePipe::install(&device, &config, _pretty_shader);
